...
export class MarketHubApiClient {
  ...
  async login(id: string, password: string): Promise<AxiosResponse<LoginResponse>> {
    const res = await this.client.post<LoginResponse>(`${API_PREFIX}/auth/login`, { id, password });
    return res;
  }

  // Users
  async getUsers(page = 1, limit = 10): Promise<AxiosResponse<PaginatedResponse<User>>> {
    const res = await this.client.get<PaginatedResponse<User>>(`${API_PREFIX}/users`, { params: { page, limit } });
    return res;
  }

  async createUser(user: CreateUserRequest): Promise<AxiosResponse<User>> {
    const res = await this.client.post<User>(`${API_PREFIX}/users`, user);
    return res;
  }

  async updateUser(id: number, data: UpdateUserRequest): Promise<AxiosResponse<User>> {
    const res = await this.client.put<User>(`${API_PREFIX}/users/${id}`, data);
    return res;
  }

  async deleteUser(id: number): Promise<AxiosResponse<void>> {
    const res = await this.client.delete<void>(`${API_PREFIX}/users/${id}`);
    return res;
  }

  // Products
  async getProducts(page = 1, limit = 10): Promise<AxiosResponse<PaginatedResponse<Product>>> {
    const res = await this.client.get<PaginatedResponse<Product>>(`${API_PREFIX}/products`, { params: { page, limit } });
    return res;
  }

  async createProduct(product: CreateProductBody): Promise<AxiosResponse<Product>> {
    const res = await this.client.post<Product>(`${API_PREFIX}/products`, product);
    return res;
  }

  async updateProduct(id: string, data: UpdateProductRequest): Promise<AxiosResponse<Product>> {
    const res = await this.client.put<Product>(`${API_PREFIX}/products/${id}`, data);
    return res;
  }

  async deleteProduct(id: string): Promise<AxiosResponse<void>> {
    const res = await this.client.delete<void>(`${API_PREFIX}/products/${id}`);
    return res;
  }

  async getProductByBarCode(barCode: string, page = 1, limit = 10): Promise<AxiosResponse<PaginatedResponse<Product>>> {
    const res = await this.client.get<PaginatedResponse<Product>>(`${API_PREFIX}/products/code/${barCode}`, { params: { page, limit } });
    return res;
  }

  // Sales
  async getSales(params: GetSalesParams = {}): Promise<AxiosResponse<PaginatedResponse<SaleWithDetails>>> {
    const res = await this.client.get<PaginatedResponse<SaleWithDetails>>(`${API_PREFIX}/sales`, { params });
    return res;
  }

  async createSale(): Promise<AxiosResponse<Sale>> {
    const res = await this.client.post<Sale>(`${API_PREFIX}/sales`);
    return res;
  }

  async getSaleById(id: string): Promise<AxiosResponse<SaleWithDetails>> {
    const res = await this.client.get<SaleWithDetails>(`${API_PREFIX}/sales/${id}`);
    return res;
  }

  async updateSale(id: string, data: UpdateSaleRequest): Promise<AxiosResponse<Sale>> {
    const res = await this.client.put<Sale>(`${API_PREFIX}/sales/${id}`, data);
    return res;
  }

  // Sale Items
  async addSaleItem(saleId: string, item: CreateSaleItemRequest): Promise<AxiosResponse<SaleItem>> {
    const res = await this.client.post<SaleItem>(`${API_PREFIX}/sales/${saleId}/items`, item);
    return res;
  }

  async updateSaleItem(saleId: string, itemId: string, data: UpdateSaleItemRequest): Promise<AxiosResponse<SaleItem>> {
    const res = await this.client.put<SaleItem>(`${API_PREFIX}/sales/${saleId}/items/${itemId}`, data);
    return res;
  }

  async deleteSaleItem(saleId: string, itemId: string): Promise<AxiosResponse<void>> {
    const res = await this.client.delete<void>(`${API_PREFIX}/sales/${saleId}/items/${itemId}`);
    return res;
  }
}